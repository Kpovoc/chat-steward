...
func (bot *Bot) handleLine(line string) {
  ...
  // Respond in Private Message if required by response, or the originating message was private
  if response.ForceWhisper || bot.nickname == channel {
    responseChan = nick
  }

  fmt.Printf("%s <- %s: %s\n", responseChan, bot.nickname, response.Content)
